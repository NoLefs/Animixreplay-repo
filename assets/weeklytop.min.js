...
function weeklyTop() {
  fetch(proxyURL + encodeURIComponent(`https://api.jikan.moe/v4/top/anime?filter=airing&limit=10`))
      ...
      ongoingplace.innerHTML = "";
			data.data.map((series) => {
				...
				const url = proxyURL +  + encodeURIComponent(`https://api.myanimelist.net/v2/anime/${malId}?fields=mean,popularity,num_list_users,num_scoring_users`);
...
function weeklyTopExpanded() {
	fetch(proxyURL +  encodeURIComponent(`https://api.myanimelist.net/v2/anime/ranking?ranking_type=airing&limit=35`))
		.then((response) => response.json())
		.then((data) => {
